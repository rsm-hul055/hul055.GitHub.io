...
And，this is a function of lambda and Y.
...
We evaluated the Poisson log-likelihood over a grid of candidate values of 𝜆. As shown in the plot below, the log-likelihood reaches its maximum at approximately 𝜆=3.68 which is equal to the sample mean of the observed patent counts. This is consistent with the theoretical property of the Poisson distribution, where the MLE ofλ is equal to the sample mean.
```{python}
import numpy as np
import matplotlib.pyplot as plt
from scipy.special import gammaln
...
```{python}
# Step 1: Copy original X matrix and modify iscustomer column to create X_0 and X_1
X_0 = X.copy()
X_1 = X.copy()

# Identify the column name for iscustomer
iscustomer_col = [col for col in X.columns if "iscustomer" in col][0]

# Set iscustomer = 0 for X_0 and 1 for X_1
X_0[iscustomer_col] = 0
X_1[iscustomer_col] = 1

# Convert to matrices for prediction
X0_matrix = X_0.values
X1_matrix = X_1.values

# Predict Poisson means (lambda) for each firm under the two scenarios
y_pred_0 = np.exp(X0_matrix @ beta_hat)
y_pred_1 = np.exp(X1_matrix @ beta_hat)

# Compute average difference in expected number of patents
